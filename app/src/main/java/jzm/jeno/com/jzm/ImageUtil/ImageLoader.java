...
public class ImageLoader {
    ...
    public static void loadRoundImageByRes(Context context, ImageView imageView, int resID, int radiusTop, int radiusBottom) {
        ...
        GlideApp.with(context)
                ...
                .diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)
                .centerCrop()
                .transform(new GlideRoundTransform(JzmApplication.getContext(), radiusTop, radiusBottom))
                .transition(DrawableTransitionOptions.withCrossFade(200))
                .into(imageView);
    }

    // 加载资源图片成高斯模糊
    public static void loadTransformByRes(final Context context, final ImageView imageView, int resId) {
        if (context == null || ((Activity) context).isFinishing()) {
            return;
        }
    ...
    public static void loadImageByPath(Context context, ImageView imageView, String path) {
        ...
        GlideApp.with(context)
                ...
                .diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)
                .centerCrop()
                .transform(new GlideRoundTransform(JzmApplication.getContext(), 5))
                .transition(DrawableTransitionOptions.withCrossFade(200))
                .into(imageView);
    }
    ...
    // 根据路径加载资源成高斯模糊
    public static void loadTransformByPath(final Context context, final ImageView imageView, String path) {
        if (context == null || ((Activity) context).isFinishing()) {
            return;
        }
...
//                .transform(new GlideRoundTransform(BaseApplication.getInstance(), 24))
    ...
    // 加载网络图片黑色渐变模糊
    public static void loadGradientByUrl(Context context, final ImageView imageView, String url) {
        if (context == null || ((Activity) context).isFinishing()) {
            return;
        }
    ...
    // 加载网络图片成高斯模糊
    public static void loadTransformByUrl(final Context context, final ImageView imageView, String url) {
        // 防止在图片加载完成之前Activity视图已经销毁造成的crash
        if (context == null || ((Activity) context).isFinishing()) {
            return;
        }
    ...
    public static void getBitmapByUrl(final Context context, String url) {
    ...
    public static void clearDiskCache() {
        GlideApp.get(JzmApplication.getContext()).clearDiskCache();
    }
    ...
    public static void saveImageToGallery(Context context, Bitmap bmp, int type) {
        ...
        // 最后通知图库更新
        String pathName = (type == CARD ? Contracts.SAVE_CARD_PATH : Contracts.SAVE_DIALOGUE_PATH);
